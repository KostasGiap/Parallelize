...
class Pi(Parallelize):
    ...
    def Calculate(self, start_iter, end_iter, q):
        pi = Decimal(0)
    ...
    def ParallelCalculation(self, iterations, q, cores):
        print "Iterations: {0}".format(iterations)
        start_iter = 0
        end_iter = iterations/cores
       
        for i in range(cores):
            self.NewThread(thread_id=i, WorkerClass=self.Calculate, arguments=(start_iter, end_iter, q))
        ...
        if iterations%cores > 0:
            ...
            self.Calculate(start_iter, end_iter, q)
            pi += q.get()
...
def main():
    ...
    for _ in range(3):
        ...
        for cores in range(1, 21):
            print "Cores: ", cores
            start = time.time()
            print "Pi: {0}".format(pi.ParallelCalculation(iterations, q, cores))
